...
uint8_t buffer[8] = { (1<<5), (1<<7), 128, 128, 128, 128, 1, 1 };
...
public:
	...
	static inline void read() {
		buffer[0] =
			(1 << 5) |
			(!digitalReadFast(BTN_S) << 4) |
			(!digitalReadFast(BTN_Y) << 3) |
			(!digitalReadFast(BTN_X) << 2) |
			(!digitalReadFast(BTN_B) << 1) |
			(!digitalReadFast(BTN_A));
		buffer[1] =
			(1 << 7) |
			(!digitalReadFast(BTN_L) << 6) |
			(!digitalReadFast(BTN_R) << 5) |
			(!digitalReadFast(BTN_Z) << 4);
